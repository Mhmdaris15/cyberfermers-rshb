...
> **Для кого это:** фермер, продающий через `svoe-rodnoe.ru`, или менеджер
> категорий, которому надо за быстро понять, что продвигать на следующей
> неделе или какой канал использовать.
>
...
- Тёмный «agri-tech» стиль, анимированное SVG-колесо календаря,
  glass-карточки.
- Одна Call to Action -кнопка → `/farmers`.
...
| Секция | Что делает |
|---|---|
| **AI Boot Sequence** | Анимация загрузки: система рекоммендаций проходит по 4 слоям (tag overlap → category → embedding → boost), показывает что считает прямо сейчас. Только при первом посещении. |
| **Executive Summary** | 3 KPI-карты — прогноз выручки на 30 дней, активных кампаний в плане, top-канал по вкладу. Числа «оживают» (анимация цифр), при изменении плана пересчитываются через SSE. |
...
| **ROI / каналы** | Детерминированный движок: ввод (скидка %, охват, конверсии) → выход (выручка, валовая прибыль, ROI %). Столбиковая программа «Channel Mix» показывает вклад каждого канала. Все коэффициенты прозрачные — см. `internal/services/recommendation/roi.go`. |
...
- **Seasonality Ring** — круговая годовая диаграмма; видно, в какие
  месяцы у фермера «отлично» по сезонности.
...
Каждый инсайт имеет одну Call To Action   — «Создать карточку с приоритетом 1» или
